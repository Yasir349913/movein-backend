...
const setupBasicProfile = validateRequest(
  Joi.object({
    date_of_birth: Joi.date().max("now").min("1900-01-01").required().messages({
...
const verifyUniversityStudent = validateRequest(
  Joi.object({
    ...
    student_id: Joi.string().min(1).max(50).required().messages({
      "string.empty": "Student ID is required",
      "string.max": "Student ID must be less than 50 characters",
    }),
    major: Joi.string().max(100).optional().allow("").messages({
...
const orderBackgroundCheck = validateRequest(
  Joi.object({
    check_type: Joi.string()
      ...
      .messages({
        "any.only":
          "Invalid background check type. Must be: background, income_employment, or social_credit",
      }),
  })
);
...
const createUniversity = validateRequest(
  Joi.object({
    ...
    domain: Joi.string()
      ...
      .messages({
        "string.pattern.base":
          "Please provide a valid domain (e.g., university.edu)",
      }),
    contact_email: Joi.string().email().required(),
    contact_phone: Joi.string()
      .pattern(/^\+?[1-9]\d{1,14}$/)
      .optional(),
    monthly_fee: Joi.number().positive().default(99.0).optional(),
    discount_percentage: Joi.number().min(0).max(100).default(15.0).optional(),

    // ✅ Fix: key define karo
    university_type: Joi.string()
      .valid("public", "private", "community", "other")
      .optional(),
  })
);
...
const approveDocument = validateRequest(
  Joi.object({
    review_notes: Joi.string().max(1000).optional().allow("").messages({
...
const createOneTimePayment = validateRequest(
  Joi.object({
    ...
    gateway: Joi.string().valid("stripe", "paypal").default("stripe").messages({
...
const confirmOneTimePayment = validateRequest(
  Joi.object({
    ...
    gateway_transaction_id: Joi.string().required().messages({
...
const createRentPayment = validateRequest(
  Joi.object({
    landlord_id: Joi.string().required().messages({
