...
onboardingProgressSchema.methods.completeStep = function (stepName) {
  ...
  const alreadyCompleted = this.steps_completed.some(
  ...
  this.completion_percentage = Math.min(
    100,
    Math.round((this.steps_completed.length / this.steps_required.length) * 100)
  );
...
onboardingProgressSchema.statics.initializeForUser = function (
  userId,
  userType
) {
  const stepFlows = {
...
export const OnboardingProgress = mongoose.model(
  "OnboardingProgress",
