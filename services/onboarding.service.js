...
import { User, OnboardingProgress, University, UniversityStudent, UserDocument, BackgroundCheck } from "../models/index.js";
...
class OnboardingService {
  ...
  async registerUser(userData) {
    const { name, email, password, user_type, phone, language_preference } = userData;
    ...
    const progress = await OnboardingProgress.initializeForUser(user._id, user_type);
  ...
  async verifyUniversityStudent(userId, universityData) {
    const { student_email, student_id, major, graduation_year } = universityData;
  ...
  async getProgressContext(userId, userType) {
    ...
    if (userType === 'tenant') {
      const universityStudent = await UniversityStudent.findOne({ user_id: userId });
      if (universityStudent) {
        context.university_verification = {
          ...
          university: await University.findById(universityStudent.university_id, 'name discount_percentage')
        };
      }
    }
    ...
    context.background_checks = {
      ...
      completed: backgroundChecks.filter(b => b.status === 'completed').length,
  ...
  getNextActions(progress) {
    ...
    return [actionMap[progress.current_step] || {
      action: 'continue',
 ...
 */
  async setupCompanyVerification(userId, companyData) {
    ...
    const requiresDocuments = ['agency', 'bank_partner'].includes(user.user_type);
    ...
    return {
      company_info: user.company_info,
      step_completed: false,
      message: 'Company information saved. Please upload required business documents.'
    };
  }
  ...
  async uploadBusinessDocument(userId, file, documentType) {
    // 1. Upload file to S3
    const fileUrl = await storageService.uploadFile(file, 'business-documents', userId);
    ...
    const hasAllRequired = requiredDocs.every(type => uploadedDocTypes.includes(type));
 ...
 */
  async setupUniversityProfile(userId, universityData) {
    ...
    const existingUniversity = await University.findOne({ admin_user_id: userId });
    ...
    const domainExists = await University.findOne({ domain: domain.toLowerCase() });
    if (domainExists) {
  ...
  async uploadUniversityLogo(userId, file, brandColors) {
    ...
    // 2. Upload logo to S3
    const logoUrl = await storageService.uploadFile(file, 'university-logos', userId);

    // 3. Parse brand colors if provided
    let parsedBrandColors = {};
    if (brandColors) {
      try {
        parsedBrandColors = typeof brandColors === 'string' ?
          JSON.parse(brandColors) : brandColors;
      } catch (error) {
  ...
  async getUniversityDashboard(userId) {
    ...
    const now = new Date();
    const trialDaysRemaining = university.subscription_status === 'trial' ?
      Math.max(0, Math.ceil((university.trial_ends_at - now) / (1000 * 60 * 60 * 24))) : 0;
    ...
    return {
      ...
      statistics: {
        ...
        engagement_rate: totalStudents > 0 ? ((verifiedStudents / totalStudents) * 100).toFixed(1) : 0
      },
      ...
      subscription_info: {
        ...
        next_billing_date: university.subscription_status === 'active' ?
